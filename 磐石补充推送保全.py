...
def check_file(item, attach_dir):
    ...
    docl = os.path.join(attach_dir, "担保资料", "担保资料.pdf")
    if not os.path.exists(docl):
        #  f"{self.item.respondent_name}-{self.item.respondent_cert_no}-担保函.pdf"
        docl = os.path.join(attach_dir, "担保资料", item.respondent_name + "-" + item.respondent_cert_no + "-保函.pdf")
...
class DoBaoQuanJob(object):
    ...
    def zhengjucailiao(self, jname, code):
        file_full_path = os.path.join(self.attach_dir, "被申请人资料", f"{self.item.respondent_name}{self.item.respondent_cert_no}")
    ...
    def danbaocailiao(self):
        ...
        file_name = f"{self.item.respondent_name}-{self.item.respondent_cert_no}-保函.pdf"
        file_full_path = os.path.join(self.attach_dir, "担保资料", file_name)
        if not os.path.exists(file_full_path):
...
if __name__ == '__main__':
    flow = XJFYBaoQuanFlow(r"C:\Users\9000\Desktop\ss\磐石诉前保全申请程序立案表头【同外网保全申请表头】 - 副本.xlsx", r'C:\Users\9000\Desktop\ss\资料样例 - 副本', '17633602179',
                           'mimashisha555...')
    flow.run()